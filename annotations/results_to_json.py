...
def px_to_feature(bbox_px, upper_left, px_size):
    ...
    feature = Feature(geometry=Polygon([[(xmin,ymin),(xmin,ymax),(xmax,ymax),(xmax,ymin),(xmin,ymin)]]))

    return feature

def convert_norm_to_px(width, height, bounding_box):
    x_center = bounding_box[0]
    y_center = bounding_box[1]
    width_bbox = bounding_box[2] / 2
    height_bbox = bounding_box[3] /2

    xmin_px = round((x_center - width_bbox) * width)
    ymin_px = round((y_center - height_bbox) * height)
    xmax_px = round((x_center + width_bbox) * width)
    ymax_px = round((y_center + height_bbox) * height)
...
def results_to_json():
    ...
    for file in files:
        ...
        lower_right_x = upper_left_x + width * px_size_x #xmax
        lower_right_y = upper_left_y - height * px_size_y # ymin

        upper_left_y, upper_left_x = transformer.transform(upper_left_y, upper_left_x)

        lower_right_y, lower_right_x = transformer.transform(lower_right_y, lower_right_x)

        px_size_x_json = (lower_right_x - upper_left_x) / width
        px_size_y_json = (upper_left_y - lower_right_y) / height
        ...
        with open(path_to_file_txt, "r") as f:
            for line in f:
                ...
                bbox_px = convert_norm_to_px(width, height, bounding_box)
                feature = px_to_feature(bbox_px, (upper_left_x, upper_left_y), (px_size_x_json, px_size_y_json))
